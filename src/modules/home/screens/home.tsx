...
const Home = () => {
  ...
  return (
    <Screen>
      <FlexContainer background="transparent" justify="center" padding="10px 15px">
        <FlexContainer background="#" align="center" justify="flex-start" gap="35px" directionwrap="column nowrap">
          <FlexContainer background="#" maxwidthcontainer="1200px" directionwrap="column nowrap" gap="5px">
            <SearchBar />
          </FlexContainer>
          {loading && <Spin indicator={<LoadingOutlined style={{ fontSize: 48 }} spin />} />}
          {categories.map((category) => (
            <FlexContainer key={category.id} background="#" maxwidthcontainer="1200px" directionwrap="column nowrap" gap="5px">
              <FlexContainer background="#" height="29px" maxwidthcontainer="1200px">
              ...
              <CarouselOriginal slidesToShow={slidesToShow}>
                ...
                {subCategories
                  ...
                  .flatMap((subcategory) =>
                    products
                      ...
                      .map((item) => <Card key={item.id} id={item.id} title={item.name} image={item.image} store={item.store} link={item.linkAffiliate} price={convertNumberToMoney(item.price)} priceOld={convertNumberToMoney(item.priceOld)} cupom={item.cupom} storeImage="assets/mercadolivre.png" />)
                  )}
                ...
                {products
                  ...
                  .map((item) => (
                    <Card key={item.id} id={item.id} title={item.name} image={item.image} store={item.store} link={item.linkAffiliate} price={convertNumberToMoney(item.price)} priceOld={convertNumberToMoney(item.priceOld)} cupom={item.cupom} storeImage="assets/mercadolivre.png" />
                  ))}
              </CarouselOriginal>
            </FlexContainer>
          ))}
